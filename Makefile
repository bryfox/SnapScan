# Build dependencies for all iOS architectures: libtiff, leptonica, tesseract
# Taken from/based on https://github.com/gali8/Tesseract-OCR-iOS/blob/master/TesseractOCR/Makefile

# Name of local lept dir (installed via git submodule)
LEPTON_NAME = leptonica
# These should match libtiff-ios Makefile
...
# XXX: For debug:
# common_cflags = -L$(LIB_FAT_DIR) -Qunused-arguments -arch $(call swap, $*, $(arch_names_all), $(archs_all)) -pipe -no-cpp-precomp -isysroot $$SDKROOT -miphoneos-version-min=$(IOS_DEPLOY_TGT) -O0 -DDEBUG -g
...
# debug
# $(info libleptfat: ${libleptfat})
# $(info liblept: ${liblept})
# $(info libleptmakefile: ${libleptmakefile})
# $(info libleptconfig: ${libleptconfig})

.PHONY : all
all : submodulestatus $(dependant_libs)

.PHONY : submodulestatus
submodulestatus :
	$(info ======= Submodule status: =======)
	@git submodule foreach --quiet 'basename `git rev-parse --show-toplevel` | tr "\n" "@"; git rev-parse --abbrev-ref HEAD' | column -t -s"@"
	@echo

# # XXX: Cleaning just lept (but keeping source changes for debugging)
# .PHONY : leptrefresh
# leptrefresh : cleanlept
...
$(libtess) : $(libtessmakefile) $(tessdownload)
...
$(libleptconfig) :
# use installed submodule instead
...
tessdownload:
	cd $(TESSERACT_SRC)
	git archive --remote $(TESS_GIT_REPO) $(TESS_GIT_BRANCH) | tar -xpf-
